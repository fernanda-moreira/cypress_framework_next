class depositPay {
    ...
    verifyHaveBonusDontWantABonus(methodImageName) {
        cy.get("iframe.w-full.h-full.bg-hex-f3f4f6")
            .should("be.visible")
            .invoke("attr", "src")
            .then((iframeUrl) => {
                ...
                cy.origin(origin, { args: { iframeUrl, methodImageName } }, ({ iframeUrl, methodImageName }) => {
                    ...
                    cy.get(`img[src*="${methodImageName}.png"]`)
                        ...
                        .click();
                    cy.get('button')
                    cy.contains('button', 'I have a promo code')
                        .should('be.visible')
                        .click();
                    cy.get('input[placeholder="Enter promo code"]')
                        .should('be.visible')
                        .type('TEST123');
                    cy.contains('div', 'I dont want a bonus')
                        .should('be.visible')
                        .click();
                })
            });
    ...
    clickBackBonus(methodImageName, eq) {
        cy.get('button.bg-color-l')
            .should('be.visible')
            .click();
        cy.get(`img[src*="${methodImageName}.png"]`)
